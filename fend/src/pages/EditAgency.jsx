...
function EditAgency() {
  ...
  const [prevagencydata, setPrevagencydata] = useState([]);

  const { id } = useParams();
  useEffect(() => {
    axios
      .get("http://localhost:8081/showagency/" + id)
      .then((res) => {
        const AgencyData = res.data[0];
  ...
  const handleSubmitEditroom = (event) => {
    ...
    axios
      .put("http://localhost:8081/editagency/" + id, {
        agencyname,
      })
      .then((res) => {
        if (res.data.Status === "OK") {
  ...
  return (
    ...
    <div className="sm:mx-auto sm:w-full sm:max-w-sm">
      <img
        ...
        alt="Your Company"
      />
      {prevagencydata.map((data, i) => (
        <h2 className="mt-10 text-center text-2xl font-bold leading-9 tracking-tight text-gray-900">
          Edit room {data.agency_name}
        </h2>
      ))}
    </div>
      ...
      <div className="Edit-page">
        <form onSubmit={handleSubmitEditroom}>
          <div className="textbox">
            <label htmlFor="email">ชื่อแผนก</label>
            <input
              type="text"
              required
